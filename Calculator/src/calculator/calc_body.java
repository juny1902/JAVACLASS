...
public class calc_body implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		btn_plus.setOnAction(event -> Plus(event));
		btn_minus.setOnAction(event -> Minus(event));
		btn_divide.setOnAction(event -> Divide(event));
		btn_prod.setOnAction(event -> Prod(event));
		btn_sign.setOnAction(event -> Sign(event));
		btn_equal.setOnAction(event -> Equal(event));
		btn_clear.setOnAction(event -> Clear(event));
	...
	public void Sign(ActionEvent event) {
		long tmp = Long.parseLong(entry);
		tmp = (-tmp);
		entry = String.valueOf(tmp);
		text_result.setText(entry);
	}
